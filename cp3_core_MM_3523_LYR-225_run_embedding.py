...
# MM-core region split:
system.split_fragment_by_identifier(identifier='225_LYR', new_names=['LYS', 'RET'],
...
core = system.get_fragments_by_identifier(identifiers=['225_RET'])

print(" ==== 225_RET ")
dict_core = {}
all_atom_charge = []
for atom in core['225_RET'].atoms:
    dict_core[atom.name] = atom.charge
    all_atom_charge.append(atom.charge)

print(dict_core)
print("sum of charges = ", sum(all_atom_charge))
print("There is no need to force 'NZ' to charge=1.0, since the sum of charges is already 1.0")

# MM, classical, LYR 472 split:
system.split_fragment_by_identifier(identifier='472_LYR', new_names=['LYS', 'LYR'],
...
RET_classical = system.get_fragments_by_identifier(identifiers=['472_LYR'])


print(" ==== 472_LYR ")
dict_RET_classical = {}
all_atom_charge = []
for atom in RET_classical['472_LYR'].atoms:
    dict_RET_classical[atom.name] = atom.charge
    all_atom_charge.append(atom.charge)

print(dict_RET_classical)
print(sum(all_atom_charge))

print("""
 All charges are 0.0
 This small fragment needs to have Charge=1.0 at CAMB3LYP/loprop-6-31+G* calculation,
 just to be consistent with the "core = LYR 472 QM-sampled" case.
 So we place a Charge=1.0 in 'C12' for instance:""")
